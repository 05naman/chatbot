...
const Chatbot = () => {
  ...
  useEffect(() => {
    ...
    setMessages([]); // Reset messages when chatbot opens
  }, []);
  ...
  //   const apiUrl = `https://api.geoapify.com/v2/places?categories=healthcare.hospital&filter=circle:${80.903993},${26.849994},5000&limit=5&apiKey=${GEOAPIFY_API_KEY}`;
  ...
  //       // Calculate distance
  //       // const distance = getDistanceFromLatLonInKm(latitude, longitude, hospitalLat, hospitalLon);
  //       const distance = getDistanceFromLatLonInKm(26.849994, 80.903993, hospitalLat, hospitalLon);
  ...
  const fetchNearbyHospitals = async () => {
    ...
    try {
      ...
      return data.features.map((hospital, index) => {
        ...
        return `${index + 1}. ${hospital.properties.name || "Unnamed Hospital"} - ${distance.toFixed(2)} km away`;
      });
    } catch (error) {
      console.error("Error fetching hospitals:", error);
      return [];
  ...
  const generateResponse = async (msg) => {
    ...
    const cleanResponse = (text) => text.replace(/\*\*/g, ""); // Remove asterisks
    const cleanedResponse = cleanResponse(result.response.text());

    setMessages((prevMessages) => [
      ...prevMessages,
      { type: "userMsg", text: msg },
      { type: "responseMsg", text: cleanedResponse },
    ]);
      ...
      {isChatbotVisible && (
        <div className="w-[400px] h-[500px] bg-white shadow-lg rounded-lg flex flex-col fixed bottom-4 right-4">
          ...
          <div className="w-full max-w-md bg-white shadow-md">
            <div className="p-4 h-96 overflow-y-auto border-b bg-gray-100 border-gray-300">
              {messages.map((msg, index) => (
                <div
                  key={index}
                  className={`mb-4 flex ${msg.type === "userMsg" ? "justify-end" : "justify-start"}`}
                >
                  <div
                    className={`p-4 rounded max-w-[70%] break-words text-base font-sans font-normal ${msg.type === "userMsg"
